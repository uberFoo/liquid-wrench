...
/// verifying (but not removing from the input) optional *opcode-extension* bits in the `ModR/M` byte.
///
/// After validating the opcode, we parse the operands for the instruction.  Doing so may require
/// parsing a `ModR/M` byte, a SIB byte, and/or bytes for immediate values, or address displacements.
...
/// implies that we need to parse the `ModR/M` byte.  However, we need to recognize the opcode byte(s)
/// first.  *TODO: Explain this later*
///
...
#[macro_export]
macro_rules! old_instr {
    ($name:ident, $inst:expr, $bytes:expr, reg($op_1:ident), ib($imm:ident)) => {
        fn $name(input: &[u8], _rex: Option<REX>) -> IResult<&[u8], Instruction> {
            let (parser, builder) = old_instr!($imm);
            do_parse!(
                input,
                tag!($bytes)
                    >> imm: parser
                    >> (Instruction {
                        opcode: $inst,
                        op_1: Some(Operand::Register($op_1())),
                        op_2: Some(builder.signed(imm)),
                        op_3: None
                    })
            )
        }
    ...
    ($name:ident, $inst:expr, $bytes:expr, modrm($op_1:ident), modrm($op_2:ident)) => {
        fn $name(input: &[u8], _rex: Option<REX>) -> IResult<&[u8], Instruction> {
            do_parse!(
                input,
                tag!($bytes)
                    >> modrm: call!(ModRM::new, _rex)
                    >> (Instruction {
                        opcode: $inst,
                        op_1: Some(modrm.$op_1()),
                        op_2: Some(modrm.$op_2()),
                        op_3: None
                    })
            )
        }
    ...
    ($name:ident, $inst:expr, [$bytes:expr] + $opcode_ext:expr, modrm($op_1:ident), ib($imm:ident)) => {
        fn $name(input: &[u8], _rex: Option<REX>) -> IResult<&[u8], Instruction> {
            let (parser, builder) = old_instr!($imm);
            do_parse!(
                input,
                tag!([$bytes])
                    >> peek!(bits!(do_parse!(
                        take_bits!(u8, 2) >> tag_bits!(u8, 3, $opcode_ext) >> ()
                    )))
                    >> modrm: call!(ModRM::new, _rex)
                    >> imm: parser
                    >> (Instruction {
    ...
    // let mut listening = server.listen(|&: request, response|
    //    self.return_page(request, response)).unwrap();
    (imm8) => {
        (le_i8, ImmediateBuilder::new(Width::Byte))
    };

    (imm32) => {
        (le_i32, ImmediateBuilder::new(Width::DWord))
    };
}
